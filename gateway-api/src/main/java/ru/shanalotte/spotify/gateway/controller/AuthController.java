...
public class AuthController {
  ...
  public @ResponseBody ResponseEntity<?> register(@Valid @RequestBody AuthDto authDto) {
    InstanceInfo userService = eurekaClient.getNextServerFromEureka("USER-SERVICE", false);
    URI uri = buildURI(userService);
    HttpClient httpClient = HttpClient.newBuilder().build();
    HttpRequest request = buildRequest(uri, objectMapper.writeValueAsString(authDto));
